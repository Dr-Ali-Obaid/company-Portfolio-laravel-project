                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 text-sm font-medium">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span
                                class="ml-1 text-gray-500 md:ml-2 dark:text-gray-400">{{ __('Add New Project') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">{{ __('Create New Project') }}
            </h1>
        </div>
    </div>
        ...
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description_ar" id="description_ar" rows="4" value="{{ old("description_ar") }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('description_ar') }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label for="description_en"
                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Project Description (English)') }}</label>
                    <textarea name="description_en" id="description_en" rows="4" value="{{ old("description_en") }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('description_en') }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                        for="image">{{ __('Project Featured Image') }}</label>
